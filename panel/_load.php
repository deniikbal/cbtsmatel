	...
	if ($pg == 'waktu') {
		...
		while ($log = mysqli_fetch_array($logQ)) {
			...
			if ($log['type'] == 'login' || $log['type'] == 'logout') {
				...
				($log['type'] == 'testongoing') ? $icon = 'fa-sign-out-alt' : $icon = 'fa-sign-out-alt';
				echo "
		...
		$logC = 0;
		echo "<ul class='timeline'><li class='time-label'><span class='bg-red' style='margin-left:10px;margin-top:15px'> " . buat_tanggal('d-m-Y', $log['date']) . " </span></li>";
